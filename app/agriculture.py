...
def fields_page(c, q):
    ...
    table_rows = "".join(
        f'<tr><td><b>{h(r["name"])}</b><div class="mut">{h(r["code"])}</div></td><td>{h(r["ownership_type"])}</td><td>{float(r["area_da"] or 0):g} da</td><td>{h(r["water_type"])}</td><td>{h(r["village"]) or "-"}</td><td>{h(r["block_no"]) or "-"} / {h(r["parcel_no"]) or "-"}</td><td>{money(r["rent_amount"]) if r["ownership_type"] == "Kiralık" else "-"}</td><td><a class="btn alt" href="/agriculture/fields?edit={r["id"]}">Düzenle</a> <form class="inline-form" method="post" action="/agriculture/field/archive" onsubmit="return confirm(\'Tarla pasife alınsın mı? Geçmiş sezonlar korunur.\')"><input type="hidden" name="id" value="{r["id"]}"><button class="btn red">Pasife Al</button></form></td></tr>'
        for r in rows
    )
    content = f"""
    <section class="agri-grid">
...
def seasons_page(c, q):
    ...
    for r in rows:
        ...
        table_rows.append(f'<tr><td>{r["season_year"]}</td><td><b>{h(r["field_name"])}</b></td><td>{h(r["crop_name"])}</td><td>{float(r["cultivated_area_da"]):g} da</td><td>{money(cost)}</td><td>{qty:,.2f} kg</td><td>{money(realized)}</td><td>{money(stock)}</td><td><b class="{"agri-positive" if profit>=0 else "agri-negative"}">{money(profit)}</b></td><td><span class="agri-status">{h(r["status"])}</span></td><td><form method="post" action="/agriculture/season/close"><input type="hidden" name="id" value="{r["id"]}"><button class="btn alt">Sezonu Kapat</button></form></td></tr>')
    content=f"""
    <section class="agri-grid">
      <div class="card"><div class="agri-card-title"><h2>🌱 Yeni Üretim Sezonu</h2></div><form method="post" action="/agriculture/season/save" class="form">
        <label>Tarla<select name="field_id" required><option value="">Seçin…</option>{field_options(fields)}</select></label><label>Sezon Yılı<input type="number" min="2000" max="2100" name="season_year" value="{date.today().year}" required></label>
        <label>Ekilecek Ürün<input name="crop_name" list="cropList" placeholder="Arpa, buğday, silajlık mısır…" required><datalist id="cropList"><option>Arpa</option><option>Buğday</option><option>Silajlık Mısır</option><option>Yonca</option><option>Fiğ</option><option>Nohut</option></datalist></label><label>Ekilen Alan (da)<input type="number" min="0.01" step="0.01" name="cultivated_area_da" required></label>
        <label>Başlangıç Tarihi<input type="date" name="start_date" value="{date.today().isoformat()}"></label><label>Hedef Verim (kg/da)<input type="number" min="0" step="0.01" name="expected_yield_kg_da"></label>
        <label>Durum<select name="status"><option>Planlandı</option><option>Devam Ediyor</option><option>Hasat Bekliyor</option></select></label><label>Not<input name="notes"></label><div class="full"><button class="btn">Sezonu Oluştur</button></div>
      </form></div>
...
def operations_page(c):
    seasons=c.execute("""SELECT s.*,f.name field_name FROM agri_seasons s JOIN agri_fields f ON f.id=s.field_id WHERE s.status<>'Kapandı' ORDER BY s.season_year DESC,f.name""").fetchall()
    ...
    input_opts=''.join(f'<option value="{r["id"]}">{h(r["category"])} · {h(r["name"])} · stok {input_balance(c,r["id"]):,.2f} {h(r["unit"])}</option>' for r in inputs)
    table_rows=''.join(f'<tr><td>{fmt_date(r["operation_date"])}</td><td><b>{h(r["field_name"])}</b><div class="mut">{h(r["crop_name"])} · {r["season_year"]}</div></td><td>{h(r["operation_type"])}</td><td>{float(r["area_da"] or 0):g} da</td><td>{money(r["total_cost"])}</td><td>{h(r["input_name"]) or "-"}</td><td>{(f"{float(r['input_qty']):g} {h(r['input_unit'])} · {money(float(r['input_qty'])*float(r['input_unit_cost']))}" if r["input_name"] else "-")}</td><td><form method="post" action="/agriculture/operation/delete" onsubmit="return confirm(\'İşlem, girdi çıkışı ve bağlı finans kaydı geri alınsın mı?\')"><input type="hidden" name="id" value="{r["id"]}"><button class="btn red">Sil / Geri Al</button></form></td></tr>' for r in rows)
    content=f"""
    <section class="agri-grid">
      <div class="card"><div class="agri-card-title"><h2>⚙️ Tarla İşlemi Kaydet</h2></div><form method="post" action="/agriculture/operation/save" class="form">
        <label>Üretim Sezonu<select name="season_id" required><option value="">Seçin…</option>{season_options(seasons)}</select></label><label>İşlem Tarihi<input type="date" name="operation_date" value="{date.today().isoformat()}" required></label>
        <label>İşlem Türü<select name="operation_type"><option>Anız Bozma</option><option>Sürme</option><option>İkileme</option><option>Diskaro / Tırmık</option><option>Ekim</option><option>Taban Gübreleme</option><option>Üst Gübreleme</option><option>İlaçlama</option><option>Sulama</option><option>Biçerdöver</option><option>Balya</option><option>Yükleme</option><option>Nakliye</option><option>Diğer</option></select></label><label>İşlenen Alan (da)<input type="number" min="0" step="0.01" name="area_da"></label>
        <label>Mazot (litre)<input type="number" min="0" step="0.01" name="fuel_liters" value="0"></label><label>Mazot ₺/litre<input type="number" min="0" step="0.01" name="fuel_unit_price" value="0"></label>
        <label>İşçilik<input type="number" min="0" step="0.01" name="labor_cost" value="0"></label><label>Dış Hizmet / Müteahhit<input type="number" min="0" step="0.01" name="contractor_cost" value="0"></label>
        <label>Diğer Maliyet<input type="number" min="0" step="0.01" name="other_cost" value="0"></label><label>Ödeme Yöntemi<select name="payment_method"><option>Nakit</option><option>Banka</option><option>Vadeli</option><option>Tahakkuk</option></select></label>
        <label class="full">Depodan Kullanılan Girdi<select name="input_id"><option value="">Girdi kullanılmadı</option>{input_opts}</select></label><label>Girdi Miktarı<input type="number" min="0" step="0.001" name="input_quantity" value="0"></label><label class="full">Not<textarea name="notes"></textarea></label>
        <div class="full agri-note">Mazot + işçilik + dış hizmet + diğer maliyet Tarım Finans'a tek gider olarak yazılır. Depodan kullanılan tohum/gübre/ilaç tekrar finans gideri oluşturmaz; yalnız sezona maliyet olarak dağıtılır.</div><div class="full"><button class="btn">İşlemi ve Maliyeti Kaydet</button></div>
      </form></div>
...
def inputs_page(c):
    ...
    purchase_rows=''.join(f'<tr><td>{fmt_date(r["tx_date"])}</td><td>{h(r["input_name"])}</td><td>{float(r["quantity"]):,.3f} {h(r["unit"])}</td><td>{money(r["unit_cost"])}</td><td>{h(r["supplier"]) or "-"}</td><td>{h(r["lot_no"]) or "-"}</td><td><form method="post" action="/agriculture/input/delete" onsubmit="return confirm(\'Alım ve bağlı Tarım Finans gideri geri alınsın mı?\')"><input type="hidden" name="id" value="{r["id"]}"><button class="btn red">Sil / Geri Al</button></form></td></tr>' for r in purchases)
    content=f"""
    <section class="agri-grid">
      <div class="card"><div class="agri-card-title"><h2>📦 Girdi Satın Al / Stok Girişi</h2></div><form method="post" action="/agriculture/input/purchase" class="form">
        <label>Girdi Adı<input name="name" list="inputNames" placeholder="Üre 46, DAP, sertifikalı arpa tohumu…" required><datalist id="inputNames">{''.join(f'<option>{h(r["name"])}</option>' for r in rows)}</datalist></label><label>Kategori<select name="category"><option>Tohum</option><option>Gübre</option><option>Zirai İlaç</option><option>Mazot</option><option>Sulama Malzemesi</option><option>Diğer</option></select></label>
        <label>Birim<select name="unit"><option>kg</option><option>litre</option><option>adet</option><option>ton</option></select></label><label>Miktar<input type="number" min="0.001" step="0.001" name="quantity" required></label>
        <label>Birim Maliyet<input type="number" min="0" step="0.01" name="unit_cost" required></label><label>Tarih<input type="date" name="tx_date" value="{date.today().isoformat()}" required></label>
        <label>Tedarikçi<input name="supplier"></label><label>Parti / Lot<input name="lot_no"></label><label>Ödeme Yöntemi<select name="payment_method"><option>Nakit</option><option>Banka</option><option>Vadeli</option></select></label><label>Not<input name="notes"></label>
        <div class="full"><button class="btn">Stok ve Tarım Gideri Oluştur</button></div>
      </form></div>
...
def harvests_page(c):
    ...
    lot_rows=[]
    for r in lots:
        bal=harvest_balance(c,r['id']);lot_rows.append(f'<tr><td>{fmt_date(r["harvest_date"])}</td><td><b>{h(r["product_name"])}</b><div class="mut">{h(r["field_name"])} · {r["season_year"]}</div></td><td>{float(r["quantity_kg"]):,.2f} kg</td><td><b>{bal:,.2f} kg</b></td><td>{float(r["moisture_pct"] or 0):g}%</td><td>{h(r["quality"]) or "-"}</td><td>{h(r["warehouse"]) or "-"}</td><td>{money(r["estimated_market_price"])}/kg</td><td>{money(bal*float(r["estimated_market_price"] or 0))}</td><td><form method="post" action="/agriculture/harvest/delete" onsubmit="return confirm(\'Hiç çıkış yoksa hasat kaydı silinecektir. Devam edilsin mi?\')"><input type="hidden" name="id" value="{r["id"]}"><button class="btn red">Sil</button></form></td></tr>')
    content=f"""
    <section class="agri-grid">
      <div class="card"><div class="agri-card-title"><h2>🌾 Hasat / Mahsul Stok Girişi</h2></div><form method="post" action="/agriculture/harvest/save" class="form">
        <label>Üretim Sezonu<select name="season_id" required><option value="">Seçin…</option>{season_options(seasons)}</select></label><label>Hasat Tarihi<input type="date" name="harvest_date" value="{date.today().isoformat()}" required></label>
        <label>Mahsul<input name="product_name" placeholder="Arpa, buğday, saman, mısır silajı…" required></label><label>Miktar (kg)<input type="number" min="0.01" step="0.01" name="quantity_kg" required></label>
        <label>Nem (%)<input type="number" min="0" max="100" step="0.01" name="moisture_pct" value="0"></label><label>Kalite / Sınıf<input name="quality"></label>
        <label>Depo<input name="warehouse" placeholder="Merkez depo, silo 1…"></label><label>Tahmini Piyasa ₺/kg<input type="number" min="0" step="0.01" name="estimated_market_price" value="0"></label>
        <label class="full">Not<textarea name="notes"></textarea></label><div class="full agri-note">Hasat, mahsul stoğunu artırır; tek başına nakit gelir oluşturmaz. Stok değeri sezonun ekonomik sonucunda ayrıca gösterilir.</div><div class="full"><button class="btn">Hasadı Stoğa Al</button></div>
      </form></div>
...
def transfers_page(c):
    ...
    transfer_rows=''.join(f'<tr><td>{fmt_date(r["transfer_date"])}</td><td>{h(r["product_name"])}</td><td>{float(r["quantity_kg"]):,.2f} kg</td><td>{money(r["unit_price"])}</td><td>{money(float(r["quantity_kg"])*float(r["unit_price"]))}</td><td>{h(r["feed_name"])}</td><td><span class="agri-status">Bağlı 3 hareket</span></td><td><form method="post" action="/agriculture/transfer/delete" onsubmit="return confirm(\'Tarım geliri, hayvancılık gideri ve yem stok girişi birlikte geri alınsın mı?\')"><input type="hidden" name="id" value="{r["id"]}"><button class="btn red">Transferi Geri Al</button></form></td></tr>' for r in transfers)
    sale_rows=''.join(f'<tr><td>{fmt_date(r["sale_date"])}</td><td>{h(r["product_name"])}</td><td>{float(r["quantity_kg"]):,.2f} kg</td><td>{money(r["unit_price"])}</td><td><b>{money(float(r["quantity_kg"])*float(r["unit_price"]))}</b></td><td>{h(r["buyer"]) or "-"}</td><td><form method="post" action="/agriculture/sale/delete" onsubmit="return confirm(\'Satış geliri ve stok çıkışı geri alınsın mı?\')"><input type="hidden" name="id" value="{r["id"]}"><button class="btn red">Satışı Geri Al</button></form></td></tr>' for r in sales)
    content=f"""
    <section class="agri-grid equal">
      <div class="card agri-transfer-box"><div class="agri-card-title"><h2>⇄ Hayvancılığa İç Transfer</h2></div><form method="post" action="/agriculture/transfer/save" class="form">
        <label>Mahsul Lotu<select id="agriTransferLot" name="harvest_lot_id" required><option value="">Seçin…</option>{lot_options(c,lots)}</select></label><label>Tarih<input type="date" name="transfer_date" value="{date.today().isoformat()}" required></label>
        <label>Miktar (kg)<input type="number" min="0.01" step="0.01" name="quantity_kg" required></label><label>İç Transfer ₺/kg<input id="agriTransferPrice" type="number" min="0.01" step="0.01" name="unit_price" required></label>
        <label class="full">Hedef Yem<select id="agriTransferFeed" name="feed_id" required><option value="">Yem Kataloğundan seçin…</option>{''.join(f'<option value="{r["id"]}">{h(r["name"])}</option>' for r in feeds)}</select></label><label class="full">Not<input name="notes"></label>
        <div class="full agri-note">Tarım iç satış geliri + hayvancılık yem gideri + yem stok girişi oluşturur. Kasa/banka hareketi yaratmaz.</div><div class="full"><button class="btn">İç Transfer Oluştur</button></div>
      </form></div>
      <div class="card"><div class="agri-card-title"><h2>🚚 Dışarı Mahsul Satışı</h2></div><form method="post" action="/agriculture/sale/save" class="form">
        <label>Mahsul Lotu<select id="agriSaleLot" name="harvest_lot_id" required><option value="">Seçin…</option>{lot_options(c,lots)}</select></label><label>Satış Tarihi<input type="date" name="sale_date" value="{date.today().isoformat()}" required></label>
        <label>Miktar (kg)<input type="number" min="0.01" step="0.01" name="quantity_kg" required></label><label>Satış ₺/kg<input id="agriSalePrice" type="number" min="0.01" step="0.01" name="unit_price" required></label>
        <label>Alıcı<input name="buyer"></label><label>Ödeme<select name="payment_method"><option>Nakit</option><option>Banka</option><option>Vadeli</option></select></label><label class="full">Not<input name="notes"></label><div class="full"><button class="btn blue">Dış Satışı Kaydet</button></div>
      </form></div>
    </section>
    <script>
    (function(){{
...
def finance_page(c):
    ...
    table_rows=''.join(f'<tr><td>{fmt_date(r["tx_date"])}</td><td>{h(r["tx_type"])}</td><td>{h(r["category"])}</td><td>{h(r["description"])}</td><td>{h(r["field_name"]) or "-"}</td><td>{(str(r["season_year"])+" · "+h(r["crop_name"])) if r["season_id"] else "-"}</td><td>{h(r["payment_method"])}</td><td><b>{money(r["amount"])}</b></td><td>{'<span class="agri-status">Otomatik</span>' if r["source_type"] else '<form method="post" action="/agriculture/finance/delete" onsubmit="return confirm(\'Tarım finans kaydı silinsin mi?\')"><input type="hidden" name="id" value="'+str(r["id"])+'"><button class="btn red">Sil</button></form>'}</td></tr>' for r in rows)
    content=f"""
    ...
    <section class="agri-grid"><div class="card"><div class="agri-card-title"><h2>💰 Diğer Tarım Gelir/Gideri</h2></div><form method="post" action="/agriculture/finance/save" class="form">
      <label>Tarih<input type="date" name="tx_date" value="{date.today().isoformat()}" required></label><label>Tür<select name="tx_type"><option>Gider</option><option>Gelir</option></select></label>
      <label>Kategori<select name="category"><option>Kira</option><option>Elektrik</option><option>Sulama</option><option>Yakıt</option><option>İşçilik</option><option>Bakım / Onarım</option><option>Nakliye</option><option>Destekleme</option><option>Mahsul Satışı</option><option>Diğer</option></select></label><label>Tutar<input type="number" min="0.01" step="0.01" name="amount" required></label>
      <label>Tarla<select name="field_id"><option value="">Genel</option>{field_options(fields)}</select></label><label>Sezon<select name="season_id"><option value="">Genel</option>{season_options(seasons)}</select></label>
      <label>Ödeme<select name="payment_method"><option>Nakit</option><option>Banka</option><option>Vadeli</option><option>Tahakkuk</option></select></label><label>Açıklama<input name="description"></label><div class="full"><button class="btn">Tarım Finansına Kaydet</button></div>
...
def render_get(c, path, q):
    ...
    if path == "/agriculture/operations": return operations_page(c)
    if path == "/agriculture/inputs": return inputs_page(c)
    if path == "/agriculture/harvests": return harvests_page(c)
    if path == "/agriculture/transfers": return transfers_page(c)
    if path == "/agriculture/finance": return finance_page(c)
...
def handle_post(c, path, f):
    ...
    if path=="/agriculture/season/save":
        field_id=as_int(f,'field_id');year=as_int(f,'season_year');crop=str(f.get('crop_name') or '').strip();area=as_float(f,'cultivated_area_da')
        ...
        c.execute("""INSERT INTO agri_seasons(field_id,season_year,crop_name,cultivated_area_da,start_date,expected_yield_kg_da,status,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?)""",(field_id,year,crop,area,f.get('start_date') or '',as_float(f,'expected_yield_kg_da'),f.get('status') or 'Planlandı',f.get('notes') or '',created))
        return '/agriculture/seasons','Üretim sezonu oluşturuldu.','Üretim sezonu oluşturdu',f'{field["name"]} · {year} · {crop}'
    ...
    if path=="/agriculture/input/purchase":
        name=str(f.get('name') or '').strip();category=str(f.get('category') or 'Diğer');unit=str(f.get('unit') or 'kg');qty=as_float(f,'quantity');unit_cost=as_float(f,'unit_cost')
    ...
    if path=="/agriculture/operation/save":
        ...
        if season['status']=='Kapandı':raise AgricultureError('Kapalı sezona yeni işlem eklenemez.')
        ...
        return '/agriculture/operations','Tarla işlemi, maliyeti ve varsa girdi tüketimi kaydedildi.','Tarla işlemi kaydetti',f'{season["field_name"]} · {op_type}'
    ...
    if path=="/agriculture/harvest/save":
        season_id=as_int(f,'season_id');season=_field_and_season(c,season_id);qty=as_float(f,'quantity_kg');product=str(f.get('product_name') or '').strip()
        ...
        c.execute("""INSERT INTO agri_harvest_lots(season_id,harvest_date,product_name,quantity_kg,moisture_pct,quality,warehouse,estimated_market_price,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)""",(season_id,harvest_date,product,qty,as_float(f,'moisture_pct'),f.get('quality') or '',f.get('warehouse') or '',as_float(f,'estimated_market_price'),f.get('notes') or '',created));lot_id=c.execute('SELECT last_insert_rowid()').fetchone()[0]
        c.execute("""INSERT INTO agri_product_transactions(harvest_lot_id,tx_date,tx_type,quantity_kg,unit_price,source_type,source_id,notes,created_at) VALUES(?,?,'Giriş',?,?,'harvest',?,?,?)""",(lot_id,harvest_date,qty,as_float(f,'estimated_market_price'),lot_id,f'{season["field_name"]} hasadı',created))
    ...
    if path in ("/agriculture/sale/save","/agriculture/transfer/save"):
        ...
        balance=harvest_balance(c,lot_id)
        if qty>balance+0.0001:raise AgricultureError(f'Mahsul stoğu yetersiz. Mevcut {balance:g} kg, istenen {qty:g} kg.')
        amount=round(qty*unit_price,2)
        if path.endswith('/sale/save'):
            tx_date=f.get('sale_date') or date.today().isoformat();desc=f'{lot["product_name"]} dış satışı · {qty:g} kg × {unit_price:g} TL'
            c.execute("""INSERT INTO agri_finance(tx_date,tx_type,category,amount,description,payment_method,field_id,season_id,source_type,created_at) VALUES(?,'Gelir','Mahsul Satışı',?,?,?,?,?,'sale',?)""",(tx_date,amount,desc,f.get('payment_method') or 'Nakit',lot['field_id'],lot['season_id'],created));finance_id=c.execute('SELECT last_insert_rowid()').fetchone()[0]
            ...
            c.execute("""INSERT INTO agri_sales(harvest_lot_id,sale_date,quantity_kg,unit_price,buyer,payment_method,finance_id,stock_tx_id,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)""",(lot_id,tx_date,qty,unit_price,f.get('buyer') or '',f.get('payment_method') or 'Nakit',finance_id,stock_tx_id,f.get('notes') or '',created));sale_id=c.execute('SELECT last_insert_rowid()').fetchone()[0]
            c.execute("UPDATE agri_finance SET source_id=? WHERE id=?",(sale_id,finance_id));c.execute("UPDATE agri_product_transactions SET source_id=? WHERE id=?",(sale_id,stock_tx_id))
            return '/agriculture/transfers','Dış satış geliri ve mahsul stok çıkışı kaydedildi.','Mahsul satışı kaydetti',desc
        ...
        c.execute("""INSERT INTO agri_internal_transfers(harvest_lot_id,transfer_date,quantity_kg,unit_price,feed_id,notes,created_at) VALUES(?,?,?,?,?,?,?)""",(lot_id,tx_date,qty,unit_price,feed_id,f.get('notes') or '',created));transfer_id=c.execute('SELECT last_insert_rowid()').fetchone()[0]
        c.execute("""INSERT INTO agri_finance(tx_date,tx_type,category,amount,description,payment_method,field_id,season_id,source_type,source_id,internal_transfer_id,created_at) VALUES(?,'Gelir','Hayvancılığa İç Transfer',?,?, 'İç Transfer',?,?,'internal_transfer',?,?,?)""",(tx_date,amount,desc,lot['field_id'],lot['season_id'],transfer_id,transfer_id,created));agri_finance_id=c.execute('SELECT last_insert_rowid()').fetchone()[0]
        ...
        c.execute("INSERT INTO feed_finance_links(feed_id,stock_tx_id,finance_id,quantity_kg,unit_price,created_at) VALUES(?,?,?,?,?,?)",(feed_id,feed_stock_id,livestock_finance_id,qty,unit_price,created))
        c.execute("""INSERT INTO agri_product_transactions(harvest_lot_id,tx_date,tx_type,quantity_kg,unit_price,source_type,source_id,notes,created_at) VALUES(?,?,'İç Transfer',?,?,'internal_transfer',?,?,?)""",(lot_id,tx_date,qty,unit_price,transfer_id,desc,created));product_tx_id=c.execute('SELECT last_insert_rowid()').fetchone()[0]
        c.execute("""UPDATE agri_internal_transfers SET agri_finance_id=?,livestock_finance_id=?,feed_stock_tx_id=?,feed_price_id=?,product_stock_tx_id=? WHERE id=?""",(agri_finance_id,livestock_finance_id,feed_stock_id,feed_price_id,product_tx_id,transfer_id))
        return '/agriculture/transfers','İç transfer tamamlandı: tarım geliri, hayvancılık yem gideri ve yem stoğu birlikte oluşturuldu.','Hayvancılığa iç transfer yaptı',desc
    ...
    if path=="/agriculture/finance/save":
        amount=as_float(f,'amount');tx_type=f.get('tx_type') or 'Gider'
        ...
        c.execute("""INSERT INTO agri_finance(tx_date,tx_type,category,amount,description,payment_method,field_id,season_id,created_at) VALUES(?,?,?,?,?,?,?,?,?)""",(f.get('tx_date') or date.today().isoformat(),tx_type,f.get('category') or 'Diğer',amount,f.get('description') or '',f.get('payment_method') or 'Nakit',field_id,season_id,created))
        return '/agriculture/finance','Tarım finans kaydı oluşturuldu.','Tarım finans kaydı oluşturdu',f'{tx_type} · {money(amount)}'
