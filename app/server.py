...
.side{top:var(--erp-top)!important;bottom:var(--erp-status)!important;width:var(--erp-side)!important;background:linear-gradient(180deg,#08683f 0%,#075c38 100%)!important;color:#fff!important;padding:72px 8px 44px!important;border-right:1px solid #075232!important;overflow-y:auto!important}.side:before{content:'🌾  ÇiftlikPro';position:absolute;top:16px;left:16px;font-size:17px;font-weight:900;color:#fff}.side a,.side summary{color:#fff!important}.side>a,.nav-group summary{padding:10px 11px!important;margin:2px 0!important;border-radius:5px!important;font-size:13px!important}.side a:hover,.side a.on,.nav-group.open-group summary{background:rgba(255,255,255,.13)!important}.nav-children{border-left:0!important;margin-left:9px!important;padding-left:8px!important}.side .nav-children a{padding:7px 10px!important;font-size:12px!important}.nav-group summary:after{font-size:16px!important}
...
body.erp-ration-reference .erp-ration-layout{display:grid;grid-template-columns:255px minmax(620px,1fr) 250px;gap:8px;align-items:start;min-width:0}
...
@media(max-width:1300px){body.erp-ration-reference .erp-ration-layout{grid-template-columns:220px minmax(560px,1fr) 220px}.erp-ration-reference .nutri-mini-grid{grid-template-columns:repeat(4,minmax(0,1fr))!important}}
...
(()=>{
 ...
 const right=document.createElement('aside'); right.className='erp-ration-right';
 const summary=document.createElement('div'); summary.className='erp-panel'; summary.innerHTML='<div class="erp-panel-head">Rasyon Özeti</div><div class="erp-panel-body"><div class="erp-summary-list" id="erp-summary-list"></div></div>';
 const safety=document.createElement('div'); safety.className='erp-panel'; safety.style.marginTop='8px'; safety.innerHTML='<div class="erp-panel-head">Güvenlik Kontrolleri</div><div class="erp-panel-body"><div class="erp-safety-list" id="erp-safety-list"></div></div>';
 right.append(summary,safety); layout.append(left,center,right);
 ...
 function sync(){
   ...
   const list=document.getElementById('erp-summary-list'); if(list) list.innerHTML=data.map((x,i)=>'<div class="erp-summary-row '+(i===5?'total':'')+'"><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('');
   const checks=[['Rumen pH',status('target-mini-ph-status')],['NDF',status('target-mini-ndf-status')],['Kaba/Kesif',status('target-mini-rc-status')],['Enerji',status('target-mini-me-status')],['Protein',status('target-mini-cp-status')],['Mineral',status('target-mini-ca-status')+' '+status('target-mini-p-status')]];
   const sl=document.getElementById('erp-safety-list'); if(sl) sl.innerHTML=checks.map(x=>{const ok=x[1].includes('Uygun');return '<div class="erp-safety-row"><span>'+x[0]+'</span><span class="erp-safety-state '+(ok?'':'warn')+'">'+(ok?'Uygun':'Kontrol')+'</span></div>'}).join('');
 }
 sync();
...
.animal-tag-btn{border-radius:5px!important;background:#f5faf7!important;border:1px solid #b9d2c2!important;padding:8px 12px!important;color:#08743b!important;font-weight:800!important;box-shadow:none!important;min-width:0!important;justify-content:center!important}
.animal-tag-btn:hover{background:#e3eee6!important;border-color:#9ebda8!important;transform:none!important}
.animal-tag-btn:before,.animal-tag-btn:after{display:none!important;content:none!important}
...
def page(title,body,path='/',user='admin',flash=''):
    ...
    nav='<a class="side-brand-home" href="/" title="Ana Sayfa">🌾 <strong>ÇiftlikPro</strong></a>'+nav_link('🏠 Dashboard','/')
...
/* FINAL UX — Bugünün İşleri + merkezi Ayarlar */
.today-work-grid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:8px}}
.today-work-card{{border:1px solid #d8e4dc;background:#fff;border-radius:8px;overflow:hidden;min-width:0}}
.today-work-head{{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:10px 12px;border-bottom:1px solid #e3ebe6;font-weight:900}}
.today-work-count{{display:inline-flex;min-width:26px;height:26px;align-items:center;justify-content:center;border-radius:13px;background:#eef6f0;color:#176b3a;font-weight:900}}
.today-work-body{{padding:10px 12px;min-height:122px}}.today-work-body .alertitem{{margin:0!important;padding:8px!important}}.today-work-actions{{margin-top:9px;display:flex;gap:7px;flex-wrap:wrap}}
.finance-today-lines{{display:grid;gap:7px}}.finance-today-lines>div{{display:flex;justify-content:space-between;gap:10px;border-bottom:1px solid #edf2ee;padding-bottom:6px}}.finance-today-lines>div:last-child{{border-bottom:0;font-weight:900}}
.settings-hub-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}}.settings-group{{border:1px solid #d9e5dd;border-radius:9px;background:#fff;padding:13px}}.settings-group h2{{font-size:15px;margin:0 0 8px}}.settings-links{{display:grid;gap:6px}}.settings-links a{{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 10px;border:1px solid #e1e9e3;border-radius:6px;text-decoration:none;color:#173c29;background:#fafcfb;font-weight:700}}.settings-links a:hover{{background:#edf7f0;border-color:#b7d1bf}}.settings-links small{{color:#738078;font-weight:500}}
.finance-bulk-summary{{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}}.finance-bulk-summary .pill{{display:flex!important;flex-direction:column;align-items:flex-start!important;gap:2px;border-radius:7px!important;padding:9px 11px!important}}.finance-bulk-summary .pill span{{font-size:10px;color:#6c7d73;font-weight:700}}.finance-bulk-summary .pill b{{font-size:15px}}.finance-bulk-summary .btn{{align-self:stretch}}
@media(max-width:900px){{.today-work-grid,.settings-hub-grid{{grid-template-columns:1fr}}.finance-bulk-summary{{grid-template-columns:1fr 1fr}}.finance-bulk-summary .btn{{grid-column:1/-1}}}}
@media(max-width:520px){{.finance-bulk-summary{{grid-template-columns:1fr}}}}

/* FINAL TODAY polish */
:root{{--erp-top:0px;--erp-cmd:58px;--erp-tabs:36px}}
.top{{display:none!important}}.erp-commandbar{{top:0!important;left:var(--erp-side)!important;padding:0 10px!important;overflow:visible!important}}.erp-command-spacer{{flex:1 1 auto;min-width:16px}}.erp-command-user{{display:flex;align-items:center;gap:14px;padding:0 8px 0 14px;white-space:nowrap;font-size:12px;color:#17251d}}.erp-command-user>a{{padding:0!important;min-width:0!important;display:inline!important;border:0!important;font-size:12px!important;font-weight:500!important}}.erp-command-user>a:hover{{background:transparent!important;text-decoration:underline}}.erp-tabs{{top:var(--erp-cmd)!important}}.layout{{padding-top:calc(var(--erp-cmd) + var(--erp-tabs))!important}}.side{{top:0!important;padding-top:62px!important}}.side:before{{display:none!important;content:none!important}}.side .side-brand-home{{position:absolute;left:8px;right:8px;top:7px;height:45px;margin:0!important;padding:0 12px!important;display:flex!important;align-items:center!important;gap:7px;border-radius:6px!important;font-size:17px!important;font-weight:900!important;background:transparent!important;color:#fff!important}}.side .side-brand-home:hover{{background:rgba(255,255,255,.12)!important}}.summary-grid .card{{height:112px!important;min-height:112px!important;padding:9px 12px!important}}.summary-grid .metric-title{{font-size:12.5px!important;line-height:1.12!important;min-height:0!important;max-height:none!important;white-space:normal!important;overflow:visible!important}}.summary-grid .stat b{{font-size:20px!important;margin:3px 0!important}}.summary-grid .metric small{{font-size:10.5px!important;line-height:1.15!important;white-space:normal!important}}.estrus-decision-badge{{padding:5px 8px!important;border-radius:7px!important;font-size:11px!important;margin:0 4px 3px 0!important}}.estrus-decision-badge small{{font-size:9px!important}}.insem-stats .card{{border-radius:7px!important}}.insem-history td,.insem-table td{{vertical-align:middle}}.insem-history .pill,.insem-table .pill{{border-radius:7px!important;padding:5px 8px!important}}
@media(max-width:900px){{.erp-commandbar{{left:0!important;overflow-x:auto!important}}.erp-command-user{{margin-left:auto;gap:9px;padding-left:8px}}.erp-command-user b{{display:none}}.side{{top:0!important;padding-top:62px!important}}.menu-toggle{{display:inline-block!important;flex:0 0 auto}}.summary-grid .card{{height:108px!important;min-height:108px!important}}}}
@media(max-width:600px){{.erp-command-user{{gap:7px}}.erp-command-user a{{font-size:11px!important}}.erp-command-user b{{display:none}}.summary-grid .metric-title{{font-size:12px!important}}.summary-grid .metric small{{font-size:10px!important}}}}
</style></head><body><div class="erp-commandbar"><button class="menu-toggle" id="menuToggle" aria-label="Menüyü aç">☰</button><a href="/animal-add"><span class="ico">＋</span>Yeni Kayıt</a><a href="/rations"><span class="ico">⚖</span>Rasyon</a><a href="/feeds"><span class="ico">🌾</span>Yem Kataloğu</a><a href="/finance"><span class="ico">₺</span>Finans</a><a href="/reports"><span class="ico">▥</span>Raporlar</a><a href="/data"><span class="ico">⇄</span>Veri</a><div class="erp-command-spacer"></div><div class="erp-command-user"><a href="/#todays-work">🔔 Bildirimler</a><a href="/settings">⚙ Ayarlar</a><b>{h(display)}</b><a href="/logout">Çıkış</a></div></div>{'' if path=='/' else f'<div class="erp-tabs"><div class="erp-tab">{h(title)}</div></div>'}<div class="layout"><aside class="side" id="sideMenu">{nav}</aside><main class="main">{fl}{body}</main></div><div class="erp-statusbar"><span>Durum: Hazır</span><span>Veritabanı: Bağlı</span><span>Aktif Kullanıcı: {h(display)}</span><span class="erp-version">v3.9.20</span></div><script>
(function(){{
...
<script>function copyLicenseDevice(){{var e=document.getElementById("license-device-id");if(!e)return;navigator.clipboard.writeText(e.innerText).then(function(){{var b=document.querySelector(".copy-license-device");b.innerText="Kopyalandı ✓";setTimeout(function(){{b.innerText="Kopyala"}},1500)}})}}</script>'''
        ...
        if path=='/settings':
            if not self.require_admin():return
            body=f'''<h1>⚙ Ayarlar</h1><p class="mut">ÇiftlikPro'nun işletme, görünüm, veri, kullanıcı ve sistem ayarlarını tek merkezden yönetin.</p>
            <div class="settings-hub-grid">
              <section class="settings-group"><h2>🏡 İşletme & Görünüm</h2><div class="settings-links">
                <a href="/farm-profile"><span>Çiftlik / İşletme Bilgileri</span><small>Ad, iletişim, veteriner, logo →</small></a>
                <a href="/?edit=1"><span>Dashboard Düzeni</span><small>Ana ekran kartları →</small></a>
              </div></section>
              <section class="settings-group"><h2>👤 Kullanıcı & Güvenlik</h2><div class="settings-links">
                <a href="/users"><span>Kullanıcı Yönetimi</span><small>Hesaplar ve roller →</small></a>
                <a href="/smtp-settings"><span>E-posta / Şifre Kurtarma</span><small>SMTP ayarları →</small></a>
                <a href="/license-info"><span>Lisans Bilgileri</span><small>Cihaz ve lisans durumu →</small></a>
              </div></section>
              <section class="settings-group"><h2>💾 Veri & Yedekleme</h2><div class="settings-links">
                <a href="/data"><span>Veri Aktarımı</span><small>JSON içe / dışa aktar →</small></a>
                <a href="/backups"><span>SQLite Yedekleri</span><small>Yedekleri görüntüle →</small></a>
                <a href="/backup/create"><span>Şimdi Yedek Al</span><small>Anlık veritabanı yedeği →</small></a>
              </div></section>
              <section class="settings-group"><h2>🐂 Besi & Program</h2><div class="settings-links">
                <a href="/performance-settings"><span>Besi Performans Ayarları</span><small>Hedef ve uyarı eşikleri →</small></a>
                <a href="/release-notes"><span>Sürüm Notları</span><small>Program değişiklikleri →</small></a>
                <a href="/#todays-work"><span>Bildirim Merkezi</span><small>Bugünün işleri →</small></a>
              </div></section>
            </div>'''
            return self.send_html(page('Ayarlar',body,'/settings',u,msg))
        if path=='/farm-profile':
            ...
            return self.send_html(page('İşletme Bilgileri',body,'/farm-profile',u,msg))
        ...
        if path=='/':
            ...
            # Desktop ERP ana ekranı: üstte 8 durum kartı, altta Bugünün İşleri.
            fixed_keys=['active_total','female','male','pregnant','calves','due','estrus','health_due']
            dashboard_summary_html=''.join(f'<div class="dashboard-slot">{dash_cards[k]}</div>' for k in fixed_keys)
            estrus_task=(estrus_dashboard_cards[0] if estrus_dashboard_cards else '<p class="mut">Bugün için kızgınlık takibi yok.</p>')
            vaccine_task=(vaccine_task_html(pregnancy_vaccines[0]) if pregnancy_vaccines else '<p class="mut">Yakın gebelik aşı görevi yok.</p>')
            body=f'''<div class="hero"><div class="farm-hero">{dashboard_logo}<div><h1>{h(farm_name)}</h1><div>ÇiftlikPro · Bugünün sürü, sağlık ve finans görünümü</div></div></div></div>
            <div class="dashboard-section-title"><h2>Çiftlik Özeti</h2><span>Canlı durum kartları</span></div>
            <div class="grid summary-grid">{dashboard_summary_html}</div>
            <div class="dashboard-section-title" id="todays-work"><h2>Bugünün İşleri</h2><span>Şu anda ilgilenmeniz gereken başlıklar</span></div>
            <div class="today-work-grid">
              <section class="today-work-card" id="approaching-estrus"><div class="today-work-head"><span>🌸 Yaklaşan Kızgınlık</span><span class="today-work-count">{len(estrus_upcoming)}</span></div><div class="today-work-body">{estrus_task}<div class="today-work-actions"><a class="btn alt" href="/estrus">Tüm Kızgınlık Takibi →</a></div></div></section>
              <section class="today-work-card"><div class="today-work-head"><span>💉 Gebelik Aşı Alarmı</span><span class="today-work-count">{len(pregnancy_vaccines)}</span></div><div class="today-work-body">{vaccine_task}<div class="today-work-actions"><a class="btn alt" href="/health">Sağlık Takibini Aç →</a></div></div></section>
              <section class="today-work-card"><div class="today-work-head"><span>₺ Finans Özeti</span><span class="today-work-count">₺</span></div><div class="today-work-body"><div class="finance-today-lines"><div><span>Toplam Gelir</span><b>{money(total_inc)}</b></div><div><span>Toplam Gider</span><b>{money(total_exp)}</b></div><div><span>Net Durum</span><b>{money(net)}</b></div></div><div class="today-work-actions"><a class="btn alt" href="/finance">Finans Detayları →</a></div></div></section>
            </div>
            <div class="card" style="display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;margin-top:12px"><div><b>Operasyon Durumu</b><div class="mut">{active_total} aktif hayvan · {pregnant} gebe · {len(estrus_upcoming)} kızgınlık takibi · {len(health_rows)} sağlık işlemi</div></div><a class="btn alt" href="/performance">Besi Performansı →</a></div>'''
            with db() as c:
            ...
            return self.send_html(page('Dashboard',body,'/',u,msg))
...
<div class="full finance-savebar"><button type="submit" class="btn" id="financeSubmitBtn">💾 Finans Kaydını Kaydet</button><span class="mut" id="financeSaveHint"></span></div></form></div></div></aside><div class="card finance-filter-card" id="financeRecords" style="margin-top:14px"><div class="finance-filter-title"><div><h2>🔎 Finans Filtreleri</h2><span class="mut">Tarih, işlem türü ve kategoriye göre kayıtları daraltın.</span></div><span class="filter-count-pill">{len(rows)} kayıt</span></div><form method="get" class="finance-toolbar finance-toolbar-modern"><label><span>📅 Başlangıç</span><input type="date" name="start" value="{h(start)}"></label><label><span>📅 Bitiş</span><input type="date" name="end" value="{h(end)}"></label><label><span>↕️ Tür</span><select name="type"><option value="">Gelir + Gider</option><option {'selected' if typ=='Gelir' else ''}>Gelir</option><option {'selected' if typ=='Gider' else ''}>Gider</option></select></label><label><span>🏷️ Kategori</span><select name="category"><option value="">Tüm Kategoriler</option>{category_opts}</select></label><div class="finance-filter-actions"><button class="btn blue">🔎 Filtrele</button><a class="btn alt" href="/finance">↺ Temizle</a><a class="btn export-btn" href="/finance/export?start={urllib.parse.quote(start)}&end={urllib.parse.quote(end)}&type={urllib.parse.quote(typ)}&category={urllib.parse.quote(category)}">⬇ CSV</a></div></form><div class="finance-table-wrap"><table class="finance-table"><tr><th>Tarih</th><th>Tür</th><th>Kategori</th><th>Açıklama</th><th>Hayvan</th><th>Durum</th><th>Ödeme</th><th>Tutar</th><th>İşlem</th></tr>{trs}</table></div></div>'''
            ...
            function syncBulkSelection(){{
              ...
              const bulkTotal=document.getElementById('bulkTotal');if(bulkTotal)bulkTotal.textContent=formatTRY(total);
              ...
              if(shareLabel) shareLabel.textContent=checks.length===1?'Satış / Kesim Bedeli':'Hayvan Başı Gelir';
