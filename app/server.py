...
APP_VERSION='3.9.10'
APP_CHANNEL='DEV'
APP_LABEL='ENTERPRISE V3.9.10 · KOMPAKT RASYON + AKILLI DENGELEME'
...
def ration_requirement_panel(rr,sm):
    ...
    def box(label,current,target,status):
        cls='ok' if 'Uygun' in status else 'warn'
        return f"<div class='nutri-mini {cls}'><span>{label}</span><b>{current}</b><small>{target}</small><em>{status}</em></div>"
    mini=''.join([
        box('KM',f"{sm['dm_kg']:.2f} kg",f"Hedef {t['dmi_kg']:.2f}",dm_s),
        box('HP',f"%{sm['cp_pct_dm']:.1f}",f"Hedef %{t['cp_pct']:.1f}",cp_s),
        box('ME',f"{sm['me_mcal']:.1f}",f"Hedef {t['me_mcal_day']:.1f} Mcal",me_s),
        box('NDF',f"%{ndf:.1f}",f"Hedef %{t['ndf_min']:.0f}–{t['ndf_max']:.0f}",ndf_s),
        box('Ca',f"{sm['ca_g']:.0f} g",f"Hedef {t['ca_g']:.0f} g",ca_s),
        box('P',f"{sm['p_g']:.0f} g",f"Hedef {t['p_g']:.0f} g",p_s),
        box('Kaba/Kesif',f"%{rough:.0f} / %{conc:.0f}",f"Kaba %{t['roughage_min']:.0f}–{t['roughage_max']:.0f}",rc_s),
    ])
    ...
    return f"""<div class='card target-card' style='margin-top:12px;border:1px solid #cfe3d5'><div class='target-head'><h3>{title}</h3>{ctx}</div><form method='post' action='/ration/target' class='target-form'><input type='hidden' name='ration_id' value='{rr['id']}'>{form_fields}<button class='btn blue compact-target-btn'>Güncelle</button></form><div class='nutri-mini-grid'>{mini}</div>{detail}<p class='mut compact-note'>Ön değerlendirmedir; nihai rasyon veteriner/zooteknist tarafından doğrulanmalıdır.</p></div>"""
...
.quick-feed-card{{padding:16px}}.quick-feed-head,.quick-feed-selected{{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}}.quick-feed-tools{{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:12px}}.quick-feed-tools>input{{flex:1;min-width:280px;padding:11px 13px;border:1px solid #cbd8cf;border-radius:10px}}.quick-feed-shortcuts{{display:flex;gap:6px;flex-wrap:wrap}}.quick-feed-results{{margin-top:10px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;max-height:290px;overflow:auto}}.quick-feed-result{{border:1px solid #dce8df;background:#fff;border-radius:10px;padding:10px 12px;text-align:left;display:flex;justify-content:space-between;gap:12px;cursor:pointer;color:inherit}}.quick-feed-result:hover,.quick-feed-result.selected{{border-color:#23824a;background:#f0f8f3}}.quick-feed-result span{{display:flex;flex-direction:column;gap:3px}}.quick-feed-result small{{color:#66736b}}.quick-feed-side{{text-align:right;white-space:nowrap}}.quick-feed-selected{{margin-top:12px;padding:11px;background:#f6faf7;border:1px solid #dce8df;border-radius:10px}}.quick-feed-selected>div:first-child{{display:flex;flex-direction:column}}.quick-feed-selected .ration-stepper input{{width:92px}}@media(max-width:760px){{.quick-feed-results{{grid-template-columns:1fr}}.quick-feed-tools>input{{min-width:100%}}.quick-feed-selected{{align-items:stretch}}.quick-feed-selected .ration-stepper{{justify-content:center}}.quick-feed-selected .btn{{justify-content:center}}}}
.target-card{{padding:14px 16px}}.target-head{{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}}.target-head h3{{margin:0}}.target-context.compact{{margin:0;padding:7px 10px;font-size:13px}}.target-form{{display:grid;grid-template-columns:repeat(3,minmax(140px,1fr)) auto;gap:8px;align-items:end;margin-top:10px}}.target-form label{{font-size:12px}}.target-form input,.target-form select{{padding:7px 9px}}.compact-target-btn{{height:36px}}.target-more{{grid-column:auto}}.target-more summary{{font-size:12px;font-weight:700;padding:8px}}.target-more-grid{{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:6px}}.nutri-mini-grid{{display:grid;grid-template-columns:repeat(7,minmax(105px,1fr));gap:7px;margin-top:10px}}.nutri-mini{{border:1px solid #dce8df;border-radius:10px;padding:8px 9px;background:#fbfdfb;min-width:0}}.nutri-mini span{{display:block;font-size:11px;color:#587064}}.nutri-mini b{{display:block;font-size:15px;line-height:1.15;margin:2px 0}}.nutri-mini small{{display:block;font-size:10px;color:#697a70;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}.nutri-mini em{{display:block;font-style:normal;font-size:10px;font-weight:700;margin-top:3px}}.nutri-mini.ok{{border-left:3px solid #27a45e}}.nutri-mini.ok em{{color:#17733d}}.nutri-mini.warn{{border-left:3px solid #e6a11a;background:#fffdf7}}.nutri-mini.warn em{{color:#a96700}}.nutri-detail{{margin-top:8px}}.nutri-detail summary{{font-size:12px;color:#345b47}}.compact-note{{margin:7px 0 0;font-size:11px}}.table-compact-wrap{{overflow:auto;margin-top:7px}}.compact-table th,.compact-table td,.ration-workbench-table th,.ration-workbench-table td,.smart-tech-table th,.smart-tech-table td{{padding:6px 8px;font-size:12px;vertical-align:middle}}.zebra tbody tr:nth-child(even),.ration-workbench-table tbody tr:nth-child(even),.smart-tech-table tbody tr:nth-child(even){{background:#f1f8f3}}.zebra tbody tr:nth-child(odd),.ration-workbench-table tbody tr:nth-child(odd),.smart-tech-table tbody tr:nth-child(odd){{background:#fff}}.ration-workbench-table tbody tr:hover,.smart-tech-table tbody tr:hover{{background:#e4f2e9}}.ration-workbench-table th:nth-child(1){{width:32%}}.ration-workbench-table th:nth-child(2){{width:22%}}.ration-workbench-table .ration-stepper{{gap:4px}}.ration-workbench-table .ration-qty{{width:72px;padding:6px}}.ration-live-grid{{gap:6px!important}}.ration-live-metric{{padding:7px 8px!important}}.smart-solution{{padding:10px}}.smart-solution .effect{{font-size:11px;color:#456859;background:#eef7f1;border-radius:7px;padding:6px 7px;margin:7px 0}}.combo-apply{{margin-top:7px}}.smart-solution-grid{{gap:8px}}.ration-section-collapse{{padding:12px 14px}}.smart-tech-table th,.smart-tech-table td{{white-space:normal}}.ration-savebar{{margin-top:7px}}.ration-page-steps{{margin-bottom:8px}}
...
setTimeout(()=>setFinanceDrawer(false),0);
    ...
    def do_POST(self):
        ...
        if path=='/ration/item':
            ...
            return self.redirect('/rations?id='+str(rid)+('&feedadd=1#quick-feed-add' if keep else ''),'Rasyon kalemi güncellendi.')
