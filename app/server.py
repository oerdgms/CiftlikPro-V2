...
APP_VERSION='3.9.20'
APP_CHANNEL='DEV'
APP_LABEL='ENTERPRISE V3.9.20 HOTFIX 3 · RASYON UX DRAWER DEV'
...
def smart_feed_bounds(feed, weight_kg, target_dm):
    """Akıllı çözücü için otomatik yem güvenlik sınırları.
    Mikro katkılar normal yem gibi kg'larla yükseltilmez. Ürün etiketi/dozu
    katalogda ayrıca tanımlı değilse konservatif prototip üst sınırları kullanılır.
    """
    dm=max(float(feed['dm_pct'] or 0)/100.0,.05); grp=feed_group(feed); name=str(feed['name'] or '').upper(); starch=float(feed['starch_pct'] or 0)
    if grp=='Katkı':
        # kg/baş/gün. Bunlar ürün etiketi bilinmeyen katalog girdileri için
        # koruyucu üst sınırlardır; çözücü hedefi tutturmak için katkı basamaz.
        if any(x in name for x in ('VİTAMİN','VITAMIN','PREMİKS','PREMIKS','PREMIX')):
            mx=0.10
        elif 'TUZ' in name:
            mx=0.10
        elif any(x in name for x in ('ÜRE','URE')):
            mx=0.10
        elif any(x in name for x in ('BİKARBONAT','BIKARBONAT')):
            mx=0.25
        elif any(x in name for x in ('FOSFAT','KİREÇ','KIREC','MİNERAL','MINERAL')):
            mx=0.20
        else:
            mx=0.15
        return 0.0,mx
...
def solve_smart_ration(feeds, weight_kg, target_adg):
    ...
    capped=[]
    for f,kg,b in zip(feeds,best,bounds):
        if feed_group(f)=='Katkı' and b[1]>0 and kg>=b[1]-0.011:
            capped.append(str(f['name']))
    if capped:warnings.append('Mikro katkı güvenlik sınırına ulaşıldı: '+', '.join(capped[:3])+('.' if len(capped)<=3 else '…'))
...
function bindRationFloatingSummary(){{
  ...
  if(!panel)return;
  const holder=document.createElement('div');holder.className='target-compare-placeholder';panel.parentNode.insertBefore(holder,panel);
  const topbar=document.querySelector('.top');
  let anchorY=0;
  function naturalTop(){{
    const was=panel.classList.contains('is-floating');
    if(was){{panel.classList.remove('is-floating');panel.style.left='';panel.style.top='';panel.style.width='';holder.classList.remove('active');holder.style.height='';}}
    anchorY=panel.getBoundingClientRect().top+window.scrollY;
  }}
  function update(){{
    if(window.innerWidth<=1180){{panel.classList.remove('is-floating');panel.style.left='';panel.style.top='';panel.style.width='';holder.classList.remove('active');holder.style.height='';return;}}
    ...
    if(trigger){{
      if(!panel.classList.contains('is-floating')){{
        const r=panel.getBoundingClientRect();holder.style.height=panel.offsetHeight+'px';holder.classList.add('active');panel.classList.add('is-floating');panel.style.left=r.left+'px';panel.style.width=r.width+'px';
      }}else{{
        const r=holder.getBoundingClientRect();panel.style.left=r.left+'px';panel.style.width=r.width+'px';
      }}
      panel.style.top=(headerH+6)+'px';
    }}else{{
      panel.classList.remove('is-floating');panel.style.left='';panel.style.top='';panel.style.width='';holder.classList.remove('active');holder.style.height='';
    }}
  }}
  naturalTop();update();
  window.addEventListener('scroll',update,{{passive:true}});
  window.addEventListener('resize',()=>{{naturalTop();update();}});
}}
